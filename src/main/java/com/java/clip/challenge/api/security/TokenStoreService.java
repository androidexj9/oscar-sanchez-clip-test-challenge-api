...
import com.java.clip.challenge.api.repository.OAuth2AccessTokenRepository;
...
public class TokenStoreService implements TokenStore {
	...
	public OAuth2Authentication readAuthentication(String tokenId) {
		...
		query.addCriteria(Criteria.where("tokenId").is(tokenId)).fields().exclude("authentication.userAuthentication");
		OAuth2AuthenticationAccessToken accessToken = mongoTemplate.findOne(query,
	...
	public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
		OAuth2AuthenticationAccessToken accessToken = new OAuth2AuthenticationAccessToken(token, authentication,
	...
	@Override
	public OAuth2AccessToken readAccessToken(String tokenId) throws InvalidTokenException {
		Query query = new Query();
		query.addCriteria(Criteria.where("tokenId").is(tokenId)).fields().exclude("authentication");
